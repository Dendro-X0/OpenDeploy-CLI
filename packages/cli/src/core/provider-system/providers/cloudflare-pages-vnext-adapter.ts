/**
 * Adapter to use the vNext @opendeploy/provider-cloudflare-pages within the
...
import { CloudflarePagesProvider as VNextProvider } from '@opendeploy/provider-cloudflare-pages'

export class CloudflarePagesVNextAdapter implements LegacyProvider {
  public readonly id: string = 'cloudflare-pages'
  ...
  // Cloudflare Pages uses wrangler; keep validateAuth a no-op for MVP
  public async validateAuth(_cwd: string): Promise<void> { return }

  // No explicit linking; just return provided or inferred project ref
