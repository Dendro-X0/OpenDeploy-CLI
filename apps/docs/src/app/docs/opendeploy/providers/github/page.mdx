...
The provider publishes the artifact directory to the `gh-pages` branch using `gh-pages` internally. No `npx`/`dlx` is required. The URL is inferred from the `origin` remote.
