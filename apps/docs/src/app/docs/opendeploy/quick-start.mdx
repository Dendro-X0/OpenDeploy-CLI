...
<DocsLayout showTableOfContents={true} tableOfContents={[
  { id: 'first-deploy', title: 'First Deploy in 30 seconds', level: 2 },
  ...
  <article className="prose prose-gray dark:prose-invert max-w-none">
    ...
    <h2 id="first-deploy">First Deploy in 30 seconds</h2>

```bash
# Non-interactive minimal defaults:
opd start --minimal
...
```bash
# Detects framework, ensures provider config (vercel.json),
